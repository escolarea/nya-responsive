...
class NewsWrapper extends Component {
  ...
  _renderExcerpts(side, page, layout) {
    ...
    const titleData = pageData.find(
      (p) => parseOnlyNumbers(p.title) === parseInt(page)
    );
    const pageStyle =
      titleData && titleData.styleName ? titleData.styleName : "front-page";
    ...
    const articles = dataToRender
      ...
      : [];
    return interpose(articles, (idx) => (
      <div className="divider" key={`divider-${idx}`} />
    ));
  }
  ...
  _renderColumnTitle(side, page) {
    ...
    const DividersWidths = pageData.dividersWidths
      ? pageData.dividersWidths.split(",").map((w) => parseInt(w))
      : null;
    let DividerWidth = DividersWidths
      ? DividersWidths[this.ColumnsToNum(side)]
      : 150;
    ...
    return (
      <div>
        <div
          className={columnTitleClasses}
          ref="content"
          dangerouslySetInnerHTML={{ __html: marked(headline) }}
        />
        {page !== 5 ||
          (page !== 6 && (
            <div style={{ width: `${DividerWidth}px` }} className="divider" />
          ))}
      </div>
    );
  }
  ...
  _renderColumn(side, page, layout) {
    ...
    const innerColumns3 =
      layout === "column-3-full" && side === "center"
        ...
        : null;
    const innerColumns4 =
      layout === "column-4-full" && side === "fifth"
        ? layoutColumns["column-4-full-inner"]
        : null;
    ...
    return (
      <div width="6" key={`column-${side}-${layout}`} className={columnCn}>
        ...
        {layout === "column-3-full-inner" &&
          side !== "sixth" &&
          this._renderColDivs("inner-column")}
        {layout === "column-4-full-inner" &&
          side !== "fourth" &&
          this._renderColDivs("inner-column")}
        {innerColumns3 &&
          innerColumns3.map((side) =>
            this._renderColumn(side, page, "column-3-full-inner")
          )}
        {innerColumns4 &&
  ...
  _renderColDivs(layout) {
    switch (layout) {
      ...
      case "column-4-full":
        return null;
      case "inner-column":
        return <div className="center-line inner-column" />;
      case "column-3-equal":
        return (
          <div>
