  ...
  const parsePlans = () => {
    ...
    for (let i = 0; i < plans.length; i++) {
      ...
      if (userPlanId == plan.product_id) {
        ...
        if (planTitle && planTitle.displayName)
          purchasedPlan.planTitle = `NYA ${planTitle.displayName.toLowerCase()}`;
      }
      let planInfo =
        (planInformation.data && getPlanInfo(planInformation.data, planType)) ||
        {};
